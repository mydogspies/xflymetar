...
public class DepartureTafHeader extends Fragment implements DataObserverIO {
    ...
    public DepartureTafHeader(VIEWSTATE state) {
        super();
        this.state = state;
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        dataViewVisibility = (TafView) MainActivity.dataViewObjects.get(VIEWSTATE.DEPARTURE_TAF);
        initElements();
    ...
    public void updateTafFromAPI(VIEWSTATE state, Taf taf) {
        if (state.equals(VIEWSTATE.DEPARTURE_TAF)) {
            if (taf.getStationID() != null) {
                headerText.setText(getString(R.string.departure_taf, taf.getStationID(), taf.getIssueTime()));
            } else {
                headerText.setText(getString(R.string.departure_taf_no_data));
            }
        }
    }
